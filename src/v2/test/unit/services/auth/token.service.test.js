...
import { GoneError, ResourceNotFoundError } from "../../../../utils/error.js";
...
describe("createResetPasswordCode", () => {
    ...
    test("should create a new reset password code", async () => {
        const user = await User.findByPk("1");
        const code = await tokenService.createResetPasswordCode(user.userID);
    ...
    test("should update the reset password code if it already exists", async () => {
        ...
        const newCode = await tokenService.createResetPasswordCode(user.userID);
...
describe("createResetPasswordSessionToken", () => {
    ...
    test("should create a new reset password session token", async () => {
        ...
        const sessionToken = await tokenService.createResetPasswordSessionToken(
            user.userID
        );
...
describe("verifyResetPasswordCode", () => {
    ...
    test("should throw an error if the code is expired", async () => {
        ...
        try {
            await tokenService.verifyResetPasswordCode(user.userID, "123456");
        } catch (err) {
