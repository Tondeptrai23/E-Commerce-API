 ...
 * Tests
 */
describe("POST /api/v2/admin/users/:userID/recover", () => {
    it("should recover a deleted user", async () => {
        ...
        const resRecover = await request(app)
            .post("/api/v2/admin/users/1/recover")
            .set("Authorization", `Bearer ${accessToken}`);
        ...
        expect(resGet.body.user.deletedAt).toBeOneOf([null, undefined]);
    });

    it("should return 404 if product is not found", async () => {
        const res = await request(app)
            .post("/api/v2/admin/users/999/recover")
            .set("Authorization", `Bearer ${accessToken}`);
    ...
    it("should return 401 if token is not provided", async () => {
        const res = await request(app).post("/api/v2/admin/users/1/recover");
